...
{
    class table2_tasks : Table_interface
    {
        ...
        public long simple_insert_rows(int row_count)
        {
            ...
            TABLE_WITH_2_COLUMNS item;
            for (int i = 0; i < row_count; i++)
            {
                item = new TABLE_WITH_2_COLUMNS { Integer1 = i , Integer2 = i+1};
                db.TABLE_WITH_2_COLUMNS.InsertOnSubmit(item);
                db.SubmitChanges();
            }
            sw.Stop();
        ...
        public long simple_update_rows(int row_count)
        {
            ...
            {
                TABLE_WITH_2_COLUMNS item = (from w in db.TABLE_WITH_2_COLUMNS where w.Id >= i select w).First();
                item.Integer1 = i + 1;
                item.Integer2 = i + 2;
                db.SubmitChanges();
            }
        ...
        public long simple_select_rows(int row_count)
        {
            ...
            TABLE_WITH_2_COLUMNS item;
            var selectQuery =
            (from w in db.TABLE_WITH_2_COLUMNS
        ...
        public long simple_delete_rows(int row_count)
        {
            ...
            for (int i = 0; i < row_count; i++)
            {
                TABLE_WITH_2_COLUMNS item = (from TABLE_WITH_2_COLUMNS w in db.TABLE_WITH_2_COLUMNS select w).First();
                db.TABLE_WITH_2_COLUMNS.DeleteOnSubmit(item);
                db.SubmitChanges();
            }
        ...
        public long simple_delete_rows_transaction(int row_count)
        {
            ...
            for (int i = 0; i < row_count; i++)
            {
                TABLE_WITH_2_COLUMNS item = (from w in db.TABLE_WITH_2_COLUMNS select w).First();
                db.TABLE_WITH_2_COLUMNS.DeleteOnSubmit(item);
            }
            db.SubmitChanges();
        ...
        public long delete_all_rows()
        {
            ...
            db.TABLE_WITH_2_COLUMNS.DeleteAllOnSubmit(selectQuery);
            db.SubmitChanges();
